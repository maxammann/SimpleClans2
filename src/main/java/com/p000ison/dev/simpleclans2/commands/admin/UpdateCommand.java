...
public class UpdateCommand extends GenericConsoleCommand {
    ...
    {
        UpdateAction action = UpdateAction.getAction(args[0]);
...
//                    double rawBalance = clan.getFlags().getDouble("balance");
//                    if (rawBalance != -1) {
//                        SimpleClans.getEconomy().createBank();
//                        clan.getFlags().removeEntry("balance");
//                    }
    ...
    private static enum UpdateAction {
        ...
        public static UpdateAction getAction(String query)
        {
            for (UpdateAction action : values()) {
                if (action.toString().equalsIgnoreCase(query)) {
                    return action;
                }
            }
