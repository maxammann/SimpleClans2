...
public class BankCommand extends GenericPlayerCommand {
    ...
    {
        super("BankCommand", plugin);
        setArgumentRange(1, 2);
        setUsages(MessageFormat.format(Language.getTranslation("usage.bank"), plugin.getSettingsManager().getClanCommand()));
        setIdentifiers(Language.getTranslation("bank.command"));
        setPermission("simpleclans.member.bank");
    }
    ...
    public String getMenu(ClanPlayer cp)
    {
        if (cp != null && cp.getClan().isVerified() && cp.isTrusted()) {
            return MessageFormat.format(Language.getTranslation("menu.bank"), plugin.getSettingsManager().getClanCommand());
        }
        return null;
    }
    ...
    public void execute(Player player, String[] args)
    {
        ...
        if (args.length == 2) {
            ...
            if (action.equalsIgnoreCase(Language.getTranslation("deposit"))) {
            ...
            } else if (action.equalsIgnoreCase(Language.getTranslation("withdraw"))) {

                if (!clan.transfer(amount, cp)) {
                    player.sendMessage(ChatColor.DARK_RED + Language.getTranslation("not.sufficient.money"));
                    return;
                }
            }
